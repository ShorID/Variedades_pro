...
@Component({
  selector: 'brand-selector-creator',
              ...
              <div class="modal-body">
                <div class="mb-3">
                  ...
                  @if (form.get('nombre')?.hasError('required')) {
                    <div class="invalid-feedback">Escoge un nombre para la marca</div>
                  }
                </div>
                <div>
...
export class BrandSelectorCreatorComponent implements OnInit {
  onClose = output();
  onSubmit = output<IInvBrand>();
  title = input<string>('');
  createForm = computed(() => {
    if (this.title()) {
  ...
  onSubmitForm(e: Event) {
    ...
    if (form && form.valid) {
      ...
      this.invHttp
        .insertBrand({ nombre: form.value.nombre || '', icono: form.value.icono || undefined })
        .pipe(
          tap((res) => {
