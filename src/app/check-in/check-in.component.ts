...
import { CurrencyPipe } from '@angular/common'; // 1. Importa el pipe
...
@Component({
  ...
  imports: [FormsModule,CurrencyPipe],//estos estaban importados TextComponent, ProductImagesComponent, ProductColorsComponent, ProductSizesComponent,
})
export class CheckInComponent implements OnInit {
  ...
  constructor(private service: VentasHttpServices, private router: Router,private usuarioauth: AuthService) {}//el router se ocupa para navegar a pagnas, router.navigate
...
confirmarCantidad() {
  ...
  if (cantidad > this.productoSeleccionado.stock) {
     ...
     else { this.productoSeleccionado.mensajeStock = '';
...
actualizarPrecio(index: number) {
  ...
  console.log(`Cambiado a ${item.empaqueSeleccionado?.nombre_empaque || 0}, nuevo precio: ${item.empaqueSeleccionado?.precio || 0}`);
...
onBuscarCliente() {
  ...
  if (termino && termino.length > 0) {
    this.sugerenciasClientes = this.listaclientes.filter(clienteb =>  
  ...
  this.service.obtenerTodos().subscribe({
    next: (data) => {
      ...
      const predeterminado = data.find(c => 
        c.nombre.toLowerCase().includes('Intimidades Jazmin')
      );
...
cerrarModal() {
  ...
  id_sucursal: 1 // ID de tu sucursal actual
    ...
    next: (res: any) => {
      ...
      error: (err) =>{ console.error('Error al procesar la venta:', err); ///alert('Error al procesar la venta')
...
/*generarPDF(facturaId: number) { this.service.descargarPDF(facturaId).subscribe(blob => {
 const url = window.URL.createObjectURL(blob);
  ...
  generarPDFRollpaper1(factura: any) {
      const doc = new jsPDF({
         orientation: 'portrait', unit: 'mm', format: [80, 200] // ancho 80mm típico de rollpaper
     });
     ...
     //  img.onload = () => { doc.addImage(img, 'PNG', 25, 5, 30, 20); // x, y, ancho, alto
      ...
      let y = 50;
        factura.detalles_factura.forEach((d: any) => {
         doc.text( `${d.producto_id} x${d.cantidad} - ${d.precio_unitario}`, 5, y ); y += 10;
        });
         doc.text('Gracias por su compra!', 5, y + 10); doc.save(`factura-${factura.id}.pdf`);
     }
  generarPDFRollpaper(factura: any) {
    if (!factura) return;
    const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: [80, 200]    // ancho típico de rollpaper
    }); // Logo (ejemplo: texto, pero puedes usar imagen base64)
      doc.setFontSize(14); doc.text('🛒 Mi Tienda', 40, 10, { align: 'center' });
      doc.setFontSize(10); doc.text(`Factura #${factura.id}`, 5, 20);
      ...
      let y = 40;
        factura.items.forEach((d: any) => {
          doc.text(`${d.producto}`, 5, y); doc.text(  `x${d.cantidad}`, 50, y);
          doc.text(`${d.precio_unitario}`, 65, y, { align: 'right' }); y += 5;
        });
          doc.line(5, y, 75, y); y += 10; doc.setFontSize(12);
          doc.text(`TOTAL: ${factura.total}`, 5, y); y += 20;
          doc.setFontSize(10);
